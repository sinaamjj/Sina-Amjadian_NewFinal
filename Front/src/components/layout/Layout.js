...
function Layout({ children, contentVariant }) {
  ...
  const routeVariant = router.pathname.startsWith("/tours/")
    ? "tour"
    : router.pathname.startsWith("/reserve/")
    ? "tour"
    : router.pathname.startsWith("/My") || router.pathname === "/Profile"
    ? "profile"
    : undefined;

  const resolvedVariant = contentVariant || routeVariant;

  const childrenClassName =
    resolvedVariant === "tour"
      ? styles.tourPage
